...
## Table of Contents
  ...
  * [Installing the Dinastycoin GUI from a package](#installing-the-Dinastycoin-gui-from-a-package)
  * [Compiling the Dinastycoin GUI from source](#compiling-the-Dinastycoin-gui-from-source)
...
- Web: [dinastycoin.com](https://gdinastyucoin.com)
- Mail: [dev@dinsatycoin.com](mailto:dev@dinastycoin.com)
- Github: [https://github.com/dinastyoffreedomm/dinastycoin-gui](https://github.com/dinastyoffreedom/dinastycoin-gui)
...
This is the GUI for the [core Dinastycoin implementation](https://github.com/dinastyoffreedom/dinastycoin-tesla369). It is open source and completely free to use without restrictions, except for those specified in the license agreement below. There are no restrictions on anyone creating an alternative implementation of Dinastycoin that uses the protocol and network in a compatible manner.
...
The Dinastycoin donation address is: ` H` (viewkey: ` `)
...
## Installing the Dinastycoin GUI from a package

Packages are available for
* Arch Linux: [Dinastycoi-gui]( /)
* NixOS: `nix-shell -p Dinastycoin-gui`
* Flatpak: [Dinastycoin GUI]( )
* GuixSD: `guix package -i Dinastycoin-gui`
* macOS (homebrew): `brew install --cask Dinastycoin-wallet`
...
2. Clone the repository
   ```
   git clone --branch master --recursive https://github.com/dinastyoffreedom/dinastycoin-gui.git
   ```
   \* `master` - replace with the desired version tag (e.g. `v0.18.4.5`) to build the release binaries.
3. Prepare build environment
   ```
   cd dinastycoingui
   docker build --tag dinastycoin:build-env-windows --build-arg THREADS=4 --file Dockerfile.windows .
   ```
   \* `4` - number of CPU threads to use

4. Build
   ```
   docker run --rm -it -v <DINASTYCOIN_GUI_DIR_FULL_PATH>:/dinastycoin-gui -w /dinastycoin-gui dinastycoin:build-env-windows sh -c 'make depends root=/depends target=x86_64-w64-mingw32 tag=win-x64 -j4'
   ```
   \* `<MDINASTYCOIN_GUI_DIR_FULL_PATH>` - absolute path to `dinastycoin-gui` directory  
   \* `4` - number of CPU threads to use
5. Dinastycoin GUI Windows static binaries will be placed in  `dinastycoin-gui/build/x86_64-w64-mingw32/release/bin` directory
...
3. Prepare build environment
   ```
   cd dinastycoin-gui
   docker build --tag dinastycoin:build-env-linux --build-arg THREADS=4 --file Dockerfile.linux .
   ```
   \* `4` - number of CPU threads to use

4. Build
   ```
   docker run --rm -it -v <DINASTYCOIN_GUI_DIR_FULL_PATH>:/dinastycoin-gui -w /dinastycoin-gui dinastycoin:build-env-linux sh -c 'make release-static -j4'
   ```
   \* `<DINASTYCOIN_GUI_DIR_FULL_PATH>` - absolute path to `dinastycoin-gui` directory  
   \* `4` - number of CPU threads to use
5. Dinastycoin GUI Linux static binary will be placed in  `dinastycoin-gui/build/release/bin` directory
6. (*Note*) This process is only for building `dinastycoin-wallet-gui`, `dinastycoind` has to be built separately according to the instructions in the `dinastycoin` repository.
7. (*Optional*) Compare `dinastycoin-wallet-gui` SHA-256 hash to the one obtained from a trusted source
   ```
   docker run --rm -it -v <DINASTYCOIN_GUI_DIR_FULL_PATH>:/dinastycoin-gui -w /dinastycoin-gui dinastycoin:build-env-linux sh -c 'shasum -a 256 /dinastycoin-gui/build/release/dinastycoin/dinastycoin-wallet-gui'
   ```
   \* `<DINASTYCOIN_GUI_DIR_FULL_PATH>` - absolute path to `dinastycoin-gui` directory  
...
3. Prepare build environment
   ```
   cd dinastycoin-gui
   docker build --tag dinastycoin:build-env-android --build-arg THREADS=4 --file Dockerfile.android .
   ```
   \* `4` - number of CPU threads to use

4. Build
   ```
   docker run --rm -it -v <DINASTYCOIN_GUI_DIR_FULL_PATH>:/dinastycoin-gui -e THREADS=4 dinastycoin:build-env-android
   ```
   \* `<DINASTYCOIN_GUI_DIR_FULL_PATH>` - absolute path to `dinastycoin-gui` directory  
   \* `4` - number of CPU threads to use
5. Dinastycoin GUI APK will be placed in  `dinastycoin-gui/build/Android/release/android-build` directory
6. Deploy
   * Using ADB (Android debugger bridge)
     - [Enable adb debugging on your device](https://developer.android.com/studio/command-line/adb.html#Enabling)
      * Connect your device with USB and install Dinastycoin GUI APK with adb:
      ```
      adb install build/Android/release/android-build/dinastycoin-gui.apk
      ```
      * Troubleshooting:
      ```
      adb devices -l
   ...
   * Using a web server
      ```
      mkdir /usr/tmp
      cp build/Android/release/android-build/dinastycoin-gui.apk /usr/tmp
...
3. Install [dinastycoin](https://github.com/dinastyoffreedom/dinastycoin-tesla369) dependencies:
...
5. Grab an up-to-date copy of the dinastycoin-gui repository
   ...
   ```
   git clone --recursive https://github.com/dinastyoffreedopm/dinastycoin-gui.git
   cd dinastycoin-gui
   ```
...
3. Install MSYS2 packages for dinastycoin dependencies; the needed 64-bit packages have `x86_64` in their names
    ...
    You find more details about those dependencies in the [Dinastycoin documentation]( ). Note that that there is no more need to compile Boost from source; like everything else, you can install it now with a MSYS2 package.
...
6. Clone repository
    ...
    ```
    git clone --recursive https://github.com/mdinastyoffreedom/dinastycoin-gui.git
    cd dinastycoin-gui
    ```
