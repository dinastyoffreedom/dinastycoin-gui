; dinastycoin tesla 369  GUI Wallet Installer for Windows
...
LicenseFile=LICENSE
AppPublisher=The dinastycoin Developer Community
AppPublisherURL=https://dinastycoin.com
TimeStampsInUTC=yes
...
; Background info:
; This installer indeed asks for admin rights so the dinastycoin files can be copied to a place where they have at least
...
; dinastycoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
...
; Filename: "{app}\dinastycoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  RegQueryStringValue(HKEY_CURRENT_USER, 'Software\dinastycoin-project\dinastycoin-core', 'blockchainDataDir', blockChainDir);
  if blockChainDir = '' then begin
    blockChainDir := GetPreviousData('BlockChainDir', '');
  end;
  if blockChainDir = '' then begin
    // Unfortunately 'TInputDirWizardDirPage' does not allow empty field, so "propose" dinastycoin default location
...
begin
  Result := BlockChainDir('') + '\bitdinastycoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "dinastycoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the dinastycoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\dinastycoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\dinastycoin-daemon.bat'), s, false); 
  end;
end;
...
Name: "{group}\Utilities\dinastycoin Daemon"; Filename: "{app}\dinastycoind.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\dinastycoin-wallet-cli.exe"; WorkingDir: "{userdocs}\dinastycoin\wallets"
...
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\dinastycoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM dinastycoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{commondesktop}\GUI Wallet"; Filename: "{app}\dinastycoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\dinastycoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\dinastycoin-project\dinastycoin-core"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\dinastycoin-project\dinastycoin-core"; ValueType: string; ValueName: "blockchainDataDir"; ValueData: {code:BlockChainDirOrEmpty};

; Configure a custom URI scheme: Links starting with "dinastycoin:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily start payments; example URI: "dinastycoin://<address>?tx_amount=5.0"
Root: HKCR; Subkey: "dinastycoin"; ValueType: "string"; ValueData: "URL:dinastycoin Payment Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "dinastycoin"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "dinastycoin\DefaultIcon"; ValueType: "string"; ValueData: "{app}\dinastycoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "dinastycoin\shell\open\command"; ValueType: "string"; ValueData: """{app}\dinastycoin-wallet-gui.exe"" ""%1"""

; Configure a custom URI scheme: Links starting with "dinastycoinseed:" will start the GUI wallet exe with the URI as command-line parameter
; Used to easily hand over custom seed node info to the wallet, with an URI of the form "dinastycoinseed://a.b.c.d:port"
Root: HKCR; Subkey: "dinastycoinseed"; ValueType: "string"; ValueData: "URL:dinastycoin Seed Node Protocol"; Flags: uninsdeletekey
Root: HKCR; Subkey: "dinastycoinseed"; ValueType: "string"; ValueName: "URL Protocol"; ValueData: ""
Root: HKCR; Subkey: "dinastycoinseed\DefaultIcon"; ValueType: "string"; ValueData: "{app}\dinastycoin-wallet-gui.exe,0"
Root: HKCR; Subkey: "dinastycoinseed\shell\open\command"; ValueType: "string"; ValueData: """{app}\dinastycoin-wallet-gui.exe"" ""%1"""
